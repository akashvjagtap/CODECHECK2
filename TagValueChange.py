...
#   - Globals are kept (Ignition gateway single-threaded callbacks).
# </summary>
...
_last_load = 0.0
_topic_name_by_id = {}            # {topic_id: canonical_name}
_cfg_by_path = {}                 # {tag_path: cfg dict}
_status_by_station = {}           # {station_root: meta}
_node_groups_by_topicstr = {}     # {resolved_topic: group dict}
_path_to_topicstrs = {}           # {leaf_path_variant: set(resolved_topics)}
_broker_cache = {"t": 0.0, "v": _DEFAULT_SERVER_NAME}
_status_last_pub_ms = {}          # {station_root: last_pub_ms}
_node_last_pub_ms   = {}          # {resolved_topic: last_pub_ms}
...
_GroupPayload = namedtuple("_GroupPayload", "payload qvs paths")
_StationSnap  = namedtuple("_StationSnap",  "relmap qvs leaves")
...
def _iter_rows(ds):
    """Yield row-like dicts from Ignition datasets or PyDataSet."""
    try:
        # PyDataSet is iterable; rows behave dict-like
        for row in ds:
            yield row
        return
    except Exception:
        pass
    try:
        # Dataset path: use column names
        rc = ds.getRowCount()
        cn = []
        try:
            for i in range(ds.getColumnCount()):
                cn.append(_u(ds.getColumnName(i)))
        except Exception:
            cn = list(ds.getColumnNames())
        for r in range(rc):
            row = {}
            for i, name in enumerate(cn):
                try:
                    row[name] = ds.getValueAt(r, name)
                except Exception:
                    try:
                        row[name] = ds.getValueAt(r, i)
                    except Exception:
                        row[name] = None
            yield row
    except Exception:
        log_error(MODULE + "::_iter_rows")
...
def _infer_topic_name(topic_id, topic):
...
def _build_row_state(row):
    """Translate a DS row to normalized fields. Returns dict or None."""
    try:
        ...
        if not path or not topic:
            return None
        tname = _infer_topic_name(topic_id, topic)
        return {"path": path, "topic": topic, "qos": qos, "retain": retain,
                "topic_id": topic_id, "config_id": cfg_id, "scope": scope, "tname": tname}
    except Exception:
        log_warn(MODULE + "::_build_row_state", None, "Skipping malformed row")
        return None


def _apply_row_state(state, cfg_by_path, status_by_station, node_groups_by_topicstr, path_to_topicstrs):
    """Update temp structures from a row state."""
    p = state["path"]
    t = state["topic"]
    q = state["qos"]
    r = state["retain"]
    ...
    cfg_by_path[p] = {"topic_id": tid, "topic": t, "qos": q, "retain": r, "config_id": cid, "scope": scope}

    if scope == u"instance" or tname == u"status" or (u"/status" in t and _station_root_of(p) == p):
        sr = _station_root_of(p)
        if sr:
            status_by_station[sr] = {"topic": t, "topic_id": tid, "qos": q, "retain": r, "config_id": cid}
        return

    grp = node_groups_by_topicstr.setdefault(
        t, {"name": tname, "topic": t, "topic_id": tid, "qos": q, "retain": r, "members": set()}
    )
    grp["members"].add(p)
    for pv in _index_variants(p):
        path_to_topicstrs.setdefault(pv, set()).add(t)
...
def _load_now():
    ...
    for row in _iter_rows(ds):
        state = _build_row_state(row)
        if state is None:
            continue
        _apply_row_state(state, cfg_by_path, status_by_station, node_groups_by_topicstr, path_to_topicstrs)

    # (Gateway tag-change execution is serialized per event type; assignment is atomic enough here)
...
def _build_payload_bool_group(members):
    ...
    return _GroupPayload(payload=payload, qvs=qvs, paths=members)
...
def _publish_status_snapshot(station_root, meta):
    try:
        snap = _relmap_from_station(station_root)
        payload = _build_status_payload(station_root, snap.relmap)
...
def _publish_node_or_cycle(topic_key, grp):
    try:
        ...
        if grp["name"] in ("faults", "andons", "alerts"):
            gp = _build_payload_bool_group(members)
        else:
            gp = _build_payload_ct_group(members)
        _publish_async(grp["topic"], grp["qos"], grp["retain"], gp.payload)
        _bulk_log_raw(grp["topic_id"], grp["qos"], grp["retain"], gp.qvs, gp.paths)
    except Exception:
...
def _handle_status_route(path):
    ...
    if sr and sr in _status_by_station:
        ...
        if (nowm - last) >= _STATUS_COALESCE_MS:
            ...
            return sr, _status_by_station.get(sr)
    return None, None


def _topic_keys_for_path(path):
    keys = _path_to_topicstrs.get(path, set())
    if not keys and path.endswith(u"/Value"):
        keys = _path_to_topicstrs.get(path[:-len(u"/Value")], set())
    elif not keys:
        keys = _path_to_topicstrs.get(path + u"/Value", set())
    return list(keys)


# ---- split _process_change into small helpers to reduce complexity ----

def _event_is_publishable(previousValue, currentValue, initialChange):
    if initialChange:
        return False
    ...
    try:
        ...
        log_error(MODULE + "::_event_is_publishable/deadband")
    return True


def _publish_status_if_needed(path):
    sr, meta = _handle_status_route(path)
    if sr and meta:
        try:
            _publish_status_snapshot(sr, meta)
        except Exception:
            log_error(MODULE + "::_publish_status_if_needed/status_publish")


def _publish_nodes_for_path(path):
    nowm = _now_ms()
    for tkey in _topic_keys_for_path(path):
        try:
            ...
            log_error(MODULE + "::_publish_nodes_for_path/node_cycle")


def _process_change(tagPath, previousValue, currentValue, initialChange):
    try:
        if not _event_is_publishable(previousValue, currentValue, initialChange):
            return
        ...
        _publish_status_if_needed(path)

        # 2) NODE/CYCLE routes
        _publish_nodes_for_path(path)
...
def _relmap_from_station(station_root):
    ...
    return _StationSnap(relmap=relmap, qvs=qvs, leaves=leaves)
...
def _detect_tt_fixtures(relmap, side_root):
...
def refresh_all():
    ...
    try:
        ...
        log_error(MODULE + "::refresh_all/userrole_cache")
    _ensure_loaded(force=True)
