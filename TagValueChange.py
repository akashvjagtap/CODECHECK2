...
#   - Globals are kept (Ignition gateway single-threaded callbacks), but writes go
#     through small helper accessors to avoid racy reads.
# </summary>
...
# These are safe in Ignition's Gateway tag-change context because execution is serialized per event;
# we still mutate via a small lock to satisfy static analyzers.
_last_load = 0.0
...
def _publish_node_or_cycle(topic_key, grp):
    try:
        ...
        if grp["name"] in ("faults", "andons", "alerts"):
            payload, qvs, paths = _build_payload_bool_group(members)
        else:
            payload, qvs, paths = _build_payload_ct_group(members)
        _publish_async(grp["topic"], grp["qos"], grp["retain"], payload)
        _bulk_log_raw(grp["topic_id"], grp["qos"], grp["retain"], qvs, paths)
    except Exception:
    ...
    try:
        ...
        if _ONLY_PUBLISH_ON_GOOD and hasattr(currentValue, "quality"):
            try:
                ...
                log_error(MODULE + "::_process_change/qualityCheck")
        ...
        try:
            ...
            log_error(MODULE + "::_process_change/deadband")
        ...
        for tkey in _topic_keys_for_path(path):
            try:
                ...
                log_error(MODULE + "::_process_change/node_cycle")
            ...
            try:
                ...
                for i in range(cnt):
                    try:
                        ...
                        log_error(MODULE + "::_extract_first_name/colname")
            except Exception:
                try:
                    ...
                    log_error(MODULE + "::_extract_first_name/colnames-fallback")
                    colnames = set()
            ...
            for col in preferred_cols:
                if col in colnames:
                    try:
                        ...
                        log_error(MODULE + "::_extract_first_name/valueAtPreferred")
            ...
            try:
                for i in range(ds.getColumnCount()):
                    try:
                        ...
                        log_error(MODULE + "::_extract_first_name/valueAtIndex")
            except Exception:
                log_error(MODULE + "::_extract_first_name/valueScan")
        ...
        for row in ds:
            for col in preferred_cols:
                try:
                    ...
                    log_error(MODULE + "::_extract_first_name/rowPreferred")
            try:
                for v in row:
                    if v is not None and _u(v).strip() != u"":
                        return _u(v)
            except Exception:
                log_error(MODULE + "::_extract_first_name/rowScan")
            break
