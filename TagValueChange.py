...
_BUCKET = namedtuple("_BUCKET", "vt vn vx vb") 
...
def _bucketize_value(val):
    if isinstance(val, bool):
        return _BUCKET(3, None, None, bool(val))
    if _is_number(val):
        return _BUCKET(1, float(val), None, None)
    if hasattr(val, "getTime"):
        return _BUCKET(4, None, system.date.format(val, "yyyy-MM-dd HH:mm:ss.SSS"), None)
    return _BUCKET(2, None, (u"%s" % val), None)
...
def _to_typed_columns(qv):
	...
	try:
	    b = _bucketize_value(val)
	except Exception:
	    log_error(MODULE + "::_to_typed_columns/bucketize")
	    b = _BUCKET(2, None, (u"%s" % val), None)
	...
	return _TYPED(vt=b.vt, vn=b.vn, vx=b.vx, vb=b.vb, ts_iso=ts_iso, qok=qok, qstr=qstr)
