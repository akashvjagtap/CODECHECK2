# ProductionTargetsLive.py
# Call run_targets() from a 5s Gateway Timer.
...
def _load_stations_if_needed():
    ...
    try:
        ds = system.db.runNamedQuery(PBASE + "getActiveStationsForTargets", {})
    except:
        ds = []
    out = []
    for r in (ds or []):
...
def _refresh_station_ct_config(sid):
    try:
        ds2 = system.db.runNamedQuery(PBASE + "getPartCTsForStation", {"station_id": sid}) or []
    except:
        ds2 = []
...
def _load_shifts_if_needed():
    ...
    def _grab(day):
        try:
            return list(system.db.runNamedQuery(PBASE + "getShiftScheduleOnDate", {"shift_date": day}) or [])
        except:
            return []
...
def _load_breaks_if_needed():
    ...
    for day in (yday, today):
        try:
            ds = system.db.runNamedQuery(PBASE + "getBreaksOnDate", {"shift_date": day})
        except:
            ds = []
        for r in (ds or []):
            try:
...
def _repair_missing_bases():
    try:
        ...
        # ----- Hours -----
        try:
            ds_h = system.db.runNamedQuery(PBASE + _NQ_GET_HOURS_MISSING_BASE,
        ...
        if up_h:
            try:
                system.db.runNamedQuery(PPATH + "upsertHourlyTargetsBatch",
        ...
        # ----- Shifts -----
        try:
            ds_s = system.db.runNamedQuery(PBASE + _NQ_GET_SHIFTS_MISSING_BASE,
        ...
        if up_s:
            try:
                system.db.runNamedQuery(PPATH + "upsertShiftTargetsBatch",
                ...
                try:
                    ...
                    if pend:
                        ...
                        if pend["ct_mode"] == "missing-config" or force_now:
                            ...
                            try:
                                system.db.runNamedQuery(PPATH + "ctSegmentUpsertOnChange", {
								    "station_id": sid,
								    "effective_from_utc": inc_ts,    # or system.date.now() in the immediate path
								    "ct_eff_sec": pend["ct_eff_sec"],
								    "fixtures_per_side": pend["fixtures_per_side"],
								    "is_turntable": pend["is_turntable"],
								    "parallelism_factor": pend["parallelism_factor"],  # <— renamed
