# ProductionPublisher.py — lean, fixed
...
def _iso_now():
    try:    return system.date.format(system.date.now(), _ISO_FMT)
    except: return system.date.format(system.date.now(), "yyyy-MM-dd HH:mm:ss.SSS")
...
def _fmt_date_local(d):   # "yyyy-MM-dd"
    try: return system.date.format(d, "yyyy-MM-dd")
    except:
        s=_u(d); return s.split("T",1)[0].split(" ",1)[0]

def _fmt_hour_HH(d):      # "HH:00"
    try: return system.date.format(d, "HH")+":00"
    except: return "00:00"

def _hour_bucket_local(d):
    try: return int(system.date.format(d,"H"))
    except: return 0
...
def _get_broker_name():
    ...
    try:
        ...
        try: names=list(system.cirruslink.engine.getServerNames() or [])
        except: names=[]
        if tag_val: chosen=tag_val
        elif names: chosen=names[0]
        else: chosen=_DEFAULT_SERVER_NAME
...
def publish_shift_rows(rows, live_snap_unused=None, qos=0, retain=False):
    ...
    for r in rows:
        ...
        # ---- stable BucketID: last elapsed hour inside the shift window ----
        #  Example: shift ends at 14:00 ⇒ anchor = 13:59:59 ⇒ BucketID = 13
...
def publish_weekly_rows(rows, qos=0, retain=False):
    ...
    for r in rows:
        ...
        payload = {
          ...
          "Timestamp": _iso_now(),
          "ProductionWeekly": {
            "Stn_ID": _u((_hier.get(sid) or {}).get("station_name") or ("Station_%d" % sid)),
            "Value":  actual
          }
        }
        try:
